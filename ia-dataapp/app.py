...
def _mlflow_plot_f1_per_class(round_num: int, worker_id: str, per_class: dict):
    ...
    ax.set_title(f"F1-score by class | Worker {worker_id} | Round {round_num}", pad=14)
...
def _mlflow_plot_confusion_matrix(round_num: int, worker_id: str, cm: list, class_names: list):
    ...
    ax.set_title(f"Normalized confusion matrix | Worker {worker_id} | Round {round_num}", pad=14)
...
def _mlflow_plot_feature_importance(round_num: int, worker_id: str, feature_importance: dict):
    ...
    ax.set_title(f"Top model features | Worker {worker_id} | Round {round_num}", pad=14)
    _mlflow_style_axes(ax, grid_axis="x")
...
def _mlflow_plot_worker_comparison(round_num: int, results: list):
    ...
    ax.set_ylim(0, 1.0)
    ax.set_ylabel("Value")
    ax.set_title(f"Local worker comparison | Round {round_num}", pad=14)
    ax.legend(ncol=2)
...
def _mlflow_plot_global_history(history: list):
    ...
    ax.set_ylabel("Value")
    ax.set_title("Global evolution of the federated model", pad=14)
    ax.set_ylim(0, 1.05)
    _mlflow_style_axes(ax, grid_axis="y")
    ax.legend()
    ...
    ax.set_xlabel("FL Round")
    ax.set_ylabel("Loss")
    ax.set_title("Global loss evolution", pad=14)
    _mlflow_style_axes(ax, grid_axis="y")
    ...
    bars = ax1.bar(rounds, elapsed, color="#6e7781", alpha=0.82, label="Time")
    ...
    ax1.set_title("Time per round and worker quorum", pad=14)
...
def _mlflow_plot_best_metrics(best_metrics: dict | None):
    ...
    ax.set_ylim(0, 1.05)
    ax.set_ylabel("Value")
    ax.set_title("Metrics of the best global model", pad=14)
...
def _mlflow_plot_transport_performance():
    ...
    ax1.set_xlabel("Registered transfer")
    ...
    ax1.set_title("FL weight transport performance", pad=14)
